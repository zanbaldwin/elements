...
Board::Board(RandomNumber &random, Console &console)
{
    ...
    // property and calling its clear() method is much more simple than writing a
    // space to each object's co-ordinates. A lazy programmer is an efficient
    // programmer; unless you're talking about speed and memory consumption... In
    // which case, you'll find me sipping pina colada's in the Caribbean after my
    // rather successful move into politics.
...
void Board::display()
{
    ...
    while(pointer != this->container.end()) {
        ...
        pointer->printAt();
        // Increment the pointer to that it points to the next object in the list.
        pointer++;
    }
}
...
void Board::moveObjects()
{
    ...
    while(pointer != this->container.end()) {
        if(this->tick % (11 - pointer->getSpeed()) == 0) {
            ...
            // remember if speed was updated too? I'LL DO THIS IF I HAVE TIME. It's
            // more than likely I'm over-complicating things. Again.
